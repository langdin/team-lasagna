...
module.exports.updateAllDocs = async (req, res, next) => {
  try {
    let updatedValues = await Request.updateMany(
      {},
      { $set: { cancelled: false } }
    );
    ...
    res
      .status(200)
      .json({ updatedValues, msg: "Successfully updated request" });
  } catch (error) {
...
module.exports.getOwnerRequestList = async (req, res, next) => {
  try {
    ...
    let reqestsData = requests.map((request) => {
      let ownerProfile = request.sitters;
      return { request, ownerProfile };
    });
    //updatedRequests.sort((a.request.start, b.request.start) = 1)
    res.status(200).json(reqestsData);
  } catch (err) {
    res.status(400).json("Unable to retreive requests");
  }
};
...
module.exports.createRequest = async (req, res, next) => {
  ...
  //console.log(userId)
  const petOwnerAccountSetUp = await Profile.findOne({
    user: userId,
    customerId: { $ne: "" },
  });
  ...
  if (!petOwnerAccountSetUp) {
    return res
      .status(404)
      .send({ msg: "Setup your Credit Card details to Create Request" });
  }
...
module.exports.completeRequest = async (req, res, next) => {
  const { start, end, completed } = req.body;

  // ensure that the end time of the request is already past
  let timeDiff = Math.floor(
    (new Date().getTime() - new Date(end).getTime()) / 60000
  );
  ...
  if (timeDiff < -5) {
    ...
    return res.status(400).json({
      msg:
        "Dog sitting shift cannot be completed until " +
