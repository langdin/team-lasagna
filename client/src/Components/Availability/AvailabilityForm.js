...
export default function Availability() {
  ...
  async function updateProfile(id, updatedValues) {
    ...
    setError(false);
    setProcessing(true);
    if (profile.hourlyRate <= 0 || !profile.accountId) {
      setError(true);
      setProcessing(false);
      return;
    }
    ...
    <form onSubmit={handleSubmitForm}>
      <Grid item xs={12} align="center">
        ...
        {error && (
          <Alert severity="error">
            Error! Please, make sure to set up hourly rate and bank account info
            to become available.
          </Alert>
        )}
        {success && (
      ...
      <Grid item xs={12}>
        ...
        <Grid
          ...
          className={classes.availableSpace}
        >
          {checkboxes.map((item) => (
            <Grid key={item.id} className={classes.item}>
              ...
              />
              <FormHelperText className={classes.checkLabel}>
                {item.label}
              </FormHelperText>
            </Grid>
          ))}
