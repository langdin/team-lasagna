  ...
  async function getbookings(sitterId) {
    try {
      const fetchedbookings = await axios.get(
        "http://localhost:3001/request/pendingBookings/" + sitterId,
    ...
    RequestList = bookings.map((booking, index) => {
      return (
        <Booking
          ...
          booking={booking}
          accept={() =>
            acceptBooking(booking.request._id, booking.request, index)
          }
          decline={() =>
            declineBooking(booking.request._id, booking.request, index)
          }
        />
      );
    });
